...
extern(C++) {
  ...
  pragma(mangle, "_Z13getStringSizeRKSs")
  size_t getStringSize(ref const std_string str);
}
...
void main() {
  ...
  assert(getStringPtr().asArray() == "string from ptr");
  assert(getStringRef().asArray() == "string from ref");
  assert(getStringCopy().asArray() == "string copy");
}
